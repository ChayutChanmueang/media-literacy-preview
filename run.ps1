...
function Configure-Env {
    ...
    if (-not (Test-Path $examplePath)) {
        ...
        $supabaseUrl = Read-Host "Enter VITE_SUPABASE_URL"
        $supabaseAnonKey = Read-Host "Enter VITE_SUPABASE_ANON_KEY"
...
do {
    ...
    switch ($selection) {
        ...
        "2" {
            ...
            Write-Success "Vite development server launched in a separate window."
        ...
        default {
            Write-Warning "Invalid option. Please choose a number from 1 to 8."
