---
name: zero-scroll-ui
description: ตรวจสอบหน้าจอมือถือแบบ "ไม่มี scroll" (single-screen UI) ด้วย audit checklist ครอบคลุม information architecture, interaction design, visual hierarchy และ technical layout — ใช้กับ component หน้าจอใน src/components/ (ConsentScreen, LandingScreen, GameShell, G1-G6*, RewardScreen, CertificateScreen, VideoScreen, LessonSelector)
trigger: /zero-scroll-ui
---
...
3. **ทดสอบ viewport matrix** (ถ้ามี dev server รันอยู่หรือใช้ `/run`, `/verify`): อย่างน้อย
...
4. **รายงานผล**: ถ้าใช้ ReportFindings tool ได้ ให้ใช้ (file/line/summary/failure_scenario, เรียงจากรุนแรงสุดก่อน) ปัญหา zero-scroll ให้ถือว่า `failure_scenario` คือ "ที่ viewport ขนาด X×Y เนื้อหาสูงเกิน แล้วเกิด scroll/ล้น" ถ้าไม่มี tool นี้ ให้สรุปเป็น bullet list ไฟล์:บรรทัด + ปัญหา + วิธีแก้แทน
