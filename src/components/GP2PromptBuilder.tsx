...
/**
 * GP2 — ขอ AI ให้ถูกคำ (AI Prompt Builder) — DROP-ZONE VARIANT — PROTOTYPE
 *
 * Gameplay variant of GP1PromptBuilder.tsx: same spec/content (reuses
 * gp1-prompt-items.json, same character_thought/result/safety_tip), but the
 * "fill the blank inside a sentence" interface is replaced with one large
 * drop area per round. Player still drags/taps the correct word in, same
 * number of rounds as GP1 (one per slot in the data — no inline sentence
 * template is rendered here). Dev-hub only, not wired into GameShell/lesson
 * flow. See docs/gdd/prototype/design-ai-prompt-builder.md for the shared
 * content/spec this variant reuses.
 */
...
export default function GP2PromptBuilder({ onFinish, logEvent }: Props) {
  ...
  useEffect(() => {
    ...
    logEvent("game_start", { game_id: "gp2" });
  }, [logEvent]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      if (currentSlotIdx >= slots.length - 1) {
        logEvent("prompt_complete", { game_id: "gp2", item_id: item.id });
        setPhase("typing");
      } else {
    ...
    return () => clearTimeout(t);
  }, [settledText, currentSlotIdx, slots.length, logEvent, item.id]);
  ...
  useEffect(() => {
    ...
    logEvent("result_view", { game_id: "gp2", item_id: item.id });
  ...
  const handlePlace = (choice: Choice) => {
    if (phase !== "building" || activeTeach || settledText) return;
    ...
    if (choice.type === "correct") {
      ...
      logEvent("slot_complete", { game_id: "gp2", item_id: item.id, slot_id: slot.id, attempts });
      setSelectedIdx(null);
      setBounce(null);
      setSettledText(choice.text);
      return;
    }
  ...
  const handleFinish = () => {
    logEvent("game_complete", { game_id: "gp2", stars, total_slots: slots.length });
    onFinish(stars);
  };
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 justify-between screen-container">
      <div className="scroll-region flex flex-col gap-4">
        {/* Header: progress dots + description popup */}
        ...
        {/* Simulated AI app screen — single drop area per round */}
        <div className="premium-card bg-[var(--bg-card)] p-4">
          ...
          <p className="text-[clamp(15px,4.2vw,20px)] font-bold text-[var(--text-primary)] mb-3">
            {currentSlot.label}
          </p>
          ...
          <div
            ...
            aria-label={`พื้นที่วางคำ: ${currentSlot.label}${
              selectedIdx !== null ? " — แตะเพื่อวางคำที่เลือก" : ""
            }`}
            className={`flex items-center justify-center min-h-[120px] rounded-[var(--radius-lg)] border-2 border-dashed text-center px-4 transition-colors ${
              settledText
                ? "border-solid border-[var(--primary)] bg-[var(--primary-light)]"
                : `border-[var(--primary)] gp1-slot-active ${
                    selectedIdx !== null ? "cursor-pointer bg-[var(--primary-light)]" : ""
                  }`
            }`}
          >
            {settledText ? (
              <span className="flex items-center gap-2 font-bold text-[var(--primary-dark)] text-[clamp(17px,4.8vw,23px)]">
                <CheckCircle2 size={26} />
                {settledText}
              </span>
            ) : (
              <span className="text-[var(--text-secondary)] text-[clamp(15px,4.2vw,20px)]">
                ลากคำมาวางที่นี่
              </span>
            )}
          </div>
        ...
        <div className="text-left">
          <span className="text-[clamp(15px,4.2vw,20px)] font-bold text-[var(--text-secondary)]">
            ลากคำที่ใช่ ใส่ในกล่องด้านบน: {currentSlot.label}
          </span>
        </div>
        ...
        <p className="text-[clamp(13px,3.7vw,17px)] text-[var(--text-secondary)] text-center">
          ลากคำ หรือ แตะคำแล้วแตะกล่องด้านบน ได้ทั้งสองแบบ
        </p>
      </div>
